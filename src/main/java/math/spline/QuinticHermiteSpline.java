...
import math.Point;
import poofs.Pose2d;
import poofs.Rotation2d;
import poofs.Translation2d;
...
public class QuinticHermiteSpline extends Spline {
  ...
  public QuinticHermiteSpline(Point p0, Point p1) {
    this(new Point2D.Double(p0.getX(), p0.getY()), p0.getHeading(), new Point2D.Double(p1.getX(), p1.getY()),
        p1.getHeading());
  }
  
  /**
   * @param p0
   *          The starting pose of the spline
   * @param p1
   *          The ending pose of the spline
   */
  public QuinticHermiteSpline(Point2D.Double p0, double headingStart, Point2D.Double p1, double headingEnd) {
    double scale = 1.2 * p0.distance(p1);
    x0 = p0.getX();
    x1 = p1.getX();
    dx0 = Math.cos(headingStart) * scale;
    dx1 = Math.cos(headingEnd) * scale;
    ddx0 = 0;
    ddx1 = 0;
    y0 = p0.getY();
    y1 = p1.getY();
    dy0 = Math.sin(headingStart) * scale;
    dy1 = Math.sin(headingEnd) * scale;
    ddy0 = 0;
    ddy1 = 0;
   ...
   */
  @Override
  public Point2D.Double getPoint2D(double t) {
   ...
   *
   * @param splines
   *          the list of splines to optimize
  ...
  private static void runOptimizationIteration(List<QuinticHermiteSpline> splines) {
    ...
    for (int i = 0; i < splines.size() - 1; ++i) {
      ...
      splines.set(i, new QuinticHermiteSpline(temp.x0, temp.x1, temp.dx0, temp.dx1, temp.ddx0, temp.ddx1 + kEpsilon,
      ...
      splines.set(i + 1, new QuinticHermiteSpline(temp1.x0, temp1.x1, temp1.dx0, temp1.dx1, temp1.ddx0, temp1.ddx1,
