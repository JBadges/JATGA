...
public class Point {
  ...
  public static double normalize(double value, double center) {
    while(value > Math.PI * 2 + (center - Math.PI)) value = (value- Math.PI*2);
    while(value < 0 + (center - Math.PI)) value = (value + Math.PI*2);
    return value;
  }

  public void normalize(double center) {
    while(getHeading() > Math.PI * 2 + (center - Math.PI)) setHeading(getHeading() - Math.PI*2);
    while(getHeading() < 0 + (center - Math.PI)) setHeading(getHeading() + Math.PI*2);
  }
